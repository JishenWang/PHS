      ...
      <div class="left-section">
        ...
        <el-card class="avatar-card" shadow="hover">
          ...
          <div class="user-basic">
            <h3 class="user-name">{{ localUserInfo.realName || localUserInfo.username || $t('profile.administrator') }}</h3>
            <p class="user-role">{{ localUserInfo.roleName || $t('profile.systemAdministrator') }}</p>
          </div>
          <!-- 统计卡片已移除 -->
        </el-card>
          ...
          <div class="security-list">
            ...
            <div class="security-item">
              ...
              <el-button type="primary" link @click="openPhoneDialog">
                {{ $t(localUserInfo.phone ? 'profile.change' : 'profile.bind') }}
              </el-button>
            </div>
            <div class="security-item">
              <div class="item-info">
              ...
              <el-button type="primary" link @click="openEmailDialog">
                {{ $t(localUserInfo.email ? 'profile.change' : 'profile.bind') }}
              </el-button>
            </div>
          </div>
        </el-card>
          ...
          <template #header>
            <div class="card-header">
              ...
              >
                ...
                {{ $t(isEditing ? 'profile.save' : 'profile.edit') }}
              </el-button>
            </div>
          </template>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item :label="$t('profile.role')">
                  <el-tag type="success">{{ formData.roleName || $t('profile.administrator') }}</el-tag>
                </el-form-item>
              </el-col>
              <el-col :span="12">
    ...
    <el-dialog v-model="showPhoneDialog" :title="phoneDialogTitle" width="450px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handlePhoneNext" :loading="changingPhone">
          {{ $t(phoneStep === 0 ? 'profile.next' : 'profile.confirmBinding') }}
        </el-button>
      </template>
    </el-dialog>
...
import { getDashboardData, updateUser } from '@/api/admin/admin'
...
const passwordRules = {
  ...
  newPassword: [
    { required: true, message: t('profile.pleaseEnterNewPassword'), trigger: 'blur' },
...
const refreshLocalUserInfo = () => {
  ...
  localUserInfo.value = {
    ...
    username: storeInfo.username || t('profile.administrator'),
    realName: storeInfo.realName || storeInfo.username || t('profile.administrator'),
    phone: storeInfo.phone || '',
    email: storeInfo.email || '',
    avatar: storeInfo.avatar || '',
    roleName: storeInfo.roleName || t('profile.systemAdministrator'),
...
const loadStatistics = async () => {
  try {
    ...
    if (res.code === 200 && res.data) {
      statistics.value = {
        userCount: res.data.totalUsers || 0,
        doctorCount: res.data.totalDoctors || 0,
        petCount: res.data.totalPets || 0
      }
    }
  } catch (error) {
...
const sendNewPhoneCode = async () => {
  if (!phoneForm.newPhone) {
    ElMessage.warning(t('profile.pleaseEnterNewPhone'))
    return
  }
