        ...
        <template v-if="activeMenu === 'notification'">
          <el-card shadow="hover" class="setting-card">
            ...
            <div class="setting-list">
              <div class="setting-item">
                <div class="item-info">
                  <div class="item-title">{{ $t('settings.newUserRegistrationAlert') }}</div>
                  <div class="item-desc">{{ $t('settings.newUserRegistrationDesc') }}</div>
                </div>
                <div class="item-control">
              ...
              <div class="setting-item">
                <div class="item-info">
                  <div class="item-title">{{ $t('settings.doctorQualificationReviewAlert') }}</div>
                  <div class="item-desc">{{ $t('settings.doctorQualificationReviewDesc') }}</div>
                </div>
                <div class="item-control">
              ...
              <div class="setting-item">
                <div class="item-info">
                  <div class="item-title">{{ $t('settings.systemExceptionAlert') }}</div>
                  <div class="item-desc">{{ $t('settings.systemExceptionDesc') }}</div>
                </div>
                <div class="item-control">
        ...
        <template v-if="activeMenu === 'data'">
          <el-card shadow="hover" class="setting-card">
            ...
            <div class="setting-list">
              ...
              <div class="setting-item danger">
                <div class="item-info">
                  <div class="item-title">{{ $t('settings.exportSettings') }}</div>
                  <div class="item-desc">{{ $t('settings.exportSettingsDesc') }}</div>
                </div>
                <div class="item-control">
                  <el-button type="primary" plain @click="handleExportData">{{ $t('settings.exportData') }}</el-button>
                </div>
              </div>
            </div>
          </el-card>
          ...
          <el-card shadow="hover" class="setting-card about-card">
            <div class="about-content">
              ...
              <el-descriptions :column="1" border class="about-info">
                <el-descriptions-item :label="$t('settings.systemName')">{{ $t('settings.systemNameAdmin') }}</el-descriptions-item>
...
import { getAdminVersion, submitAdminFeedback } from '@/api/admin/admin'
...
const handleExportData = () => {
  ...
  a.href = url
  a.download = `admin_settings_${new Date().getTime()}.json`
  a.click()
...
const handleCheckUpdate = async () => {
  ...
  try {
    const res = await getAdminVersion()
    if (res.code === 200 && res.data) {
      Object.assign(versionInfo, res.data)
      if (res.data.forceUpdate) {
        ElMessageBox.alert(res.data.updateLog || t('settings.newVersionFound'), t('settings.versionUpdate'), {
...
const handleSubmitFeedback = async () => {
  ...
  try {
    const res = await submitAdminFeedback({
    ...
    if (res.code === 200) {
      ...
      ElMessage.error(res.message || t('settings.submitFailed'))
    }
  } catch (error) {
    ElMessage.error(t('settings.submitFailed'))
  } finally {
